...
def key_generation():
    ...
    seed(1)
    for _ in range(1):
        u = Crypto.Util.number.getRandomRange(1, pow(2,30), randfunc=None) 
        gamma = Crypto.Util.number.getRandomRange(1, pow(2,30), randfunc=None)
    ...
    for _ in range(1):
        x = Crypto.Util.number.getRandomRange(1, n, randfunc=None)
...
for _ in range(N):
    ...
    for _ in range(N):
        ...
        print('u = %d' %(int(str(u)[:20])))
        print('gamma = %d' %(int(str(gamma)[:20])))
        print('v = %d...' %(int(str(v)[:20])))
        print('lambda = %d...' %(int(str(Lambda)[:20])))
        print('omega = %d...' %(int(str(omega)[:20])))
        ...
        print('signature = %d...' %(int(str(z)[:20])))
        print('message = %s.' %M)
        ...
        print('Time taken for key generation operation %s is %.7lfs '%(j,time_KeyGen))
